...
function openWindow(infowindow, marker, capacity, max) {
    let bkgcolor = capacity < max ? "white" : "red";
    let full = capacity < max ? "" : "(FULL)";
    const content = `<div style="background-color:${bkgcolor};padding:10px;"><h1>${marker.title}</h1><h2>Capcity ${capacity}/${max} ${full}</h2></div>`;
